    ...
    <div class="grid grid-cols-2 p-3 bg-white" >
      ...
      <div>
        ...
        <div class="flex justify-end m-2 p-2">
          ...
          <input data-bs-toggle="tooltip" data-bs-placement="top" title="Search with Institute , Year or Month" wire:click="gotoPage(1)" type="search" wire:model="search" placeholder="Search...  " class="block w-64  transition duration-150 ease-in-out appearance-none bg-white border border-sky-700 rounded-md py-2 px-3 text-base leading-normal sm:text-sm sm:leading-5">

        </div>
        <div class="flex justify-end m-2 p-2">
         
          <x-jet-button wire:click="showInsModal"  class="bg-sky-800 hover:bg-sky-700 m-2">Create New Institute </x-jet-button> 
          <x-jet-button wire:click="back"  class="bg-sky-800 hover:bg-sky-700 m-2">Back </x-jet-button> &nbsp;
          
        </div>
      </div>

      <div class="p-1 text-sky-800">
      @foreach (\App\Models\institute::where('id','=',$this->insid)->get() as $data)
      <strong>Main Institute - </strong>{{ $data->institute }} - ({{  $data->institute_code }})
      @endforeach
      </div>  
                  ...
                  <thead class="bg-sky-800 dark:bg-gray-600 dark:text-gray-200">
                    <tr>
                      ...
                      <th scope="col" class="px-6 py-4 whitespace-nowrap text-white w-4">Index</th>
                       <th scope="col" class="px-6 py-4 whitespace-nowrap text-white w-8">Sub Institute Code </br> උප ආයතන</th>
                       <th scope="col" class="px-6 py-4 whitespace-nowrap text-white">Sub Institute </br> උප ආයතනය</th>
              ...
              <div >
                  <form enctype="multipart/form-data">
                    ...
                    <div class="sm:col-span-6">
                      <label for="institute_code" class="block text-sm font-medium text-sky-800"> Institute / ආයතනය </label>
                      <div class="mt-1">
                        
                        <select name="main_institute"  wire:model.lazy="main_institute" readonly class=" block mt-1 w-full border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm">
                          
                          
                          <div class="hidden">
                            {{
                              $inst=DB::table("institute")
                              ->where('id','=',$this->insid)
                          ...
                          <option  value="{{$this->insid}}">{{ $data->institute_code }} - {{ $data->institute }}</option>
                          @endforeach
                      ...
                      @error('main_institute') <span class="text-red-400">{{ $message }}</span> @enderror
                    </div>
                    ...
                    <div class="sm:col-span-6">
                      <label for="sub_institute_code" class="block text-sm font-medium text-sky-800">Sub Institute Code / උප ආයතන  කේතය </label>
                      <div class="mt-1">
                    ...
                    <div class="sm:col-span-6">
                      <label for="institute" class="block text-sm font-medium text-sky-800">Sub Institute / උප ආයතනය  </label>
                      <div class="mt-1">
                        <input type="text" id="institute" wire:model.lazy="institute" name="sub_institute" class="block text-sky-800 w-full  appearance-none bg-white border border-gray-400 rounded-md py-2 px-3 border-sky-700 focus:border-sky-400 focus:ring focus:ring-sky-200 focus:ring-opacity-50 text-base leading-normal transition duration-150 ease-in-out sm:text-sm sm:leading-5" />
                      </div>
